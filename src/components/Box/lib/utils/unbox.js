  ...
  return Promise.resolve().then(function() {
    ...
    if (contents.length) {
      var err = "Something already exists at the destination. " +
                "`sun-photon init` and `sun-photon unbox` must be executed in an empty folder. " +
  ...
  return new Promise(function(accept, reject) {
    ...
    request(options, function(error, r) {
      if (error) {
        ...
        return reject(new Error("sun-photon Box at URL " + url + " doesn't exist. If you believe this is an error, please contact sun-photon support."));
...
function readBoxConfig(destination) {
  var possibleConfigs = [
    path.join(destination, "sun-photon.json"),
    path.join(destination, "sun-photon-init.json")
  ];
...
function cleanupUnpack(boxConfig, destination) {
  ...
  needingRemoval.push("sun-photon.json");
  needingRemoval.push("sun-photon-init.json");
