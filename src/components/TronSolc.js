...
function getWrapper(options = {}) {
  ...
  let solcDir = path.join(homedir(), '.sunphoton', 'solc');
  ...
  if (options.networks) {
    ...
    try {
      ...
      if (supportedVersions.includes(version)) {
        ...
        console.error(`Error:
Sun-Photon supports only the following versions:
...
`)
  ...
  if (!fs.existsSync(soljsonPath)) {
    if (process.env.SUNPHOTON_NAME) {
      name = process.env.SUNPHOTON_NAME
    }
