...
function filterNetworkConfig(options) {
  ...
  return {
    fullNode: options.fullNode || options.fullHost || options.mainFullHost,
    mainFullHost: options.mainFullHost,
    sideFullHost: options.sideFullHost,
    mainGateway: options.mainGateway,
    sideGateway: options.sideGateway,
...
function init(options, extraOptions) {
  ...
  if(extraOptions.verify && (
    !options || !options.privateKey || !(
      options.fullHost || options.mainFullHost || (options.fullNode && options.solidityNode && options.eventServer)
    )
  )) {
    if (!options) {
      throw new Error('It was not possible to instantiate TronWeb. The chosen network does not exist in your "sun-photon.js".')
    } else {
      throw new Error('It was not possible to instantiate TronWeb. Some required parameters are missing in your "sun-photon.js".')
    }
  }

  const mainchain = new _TronWeb(
    options.fullNode || options.mainFullHost,
    options.solidityNode || options.mainFullHost,
    options.eventServer || options.mainFullHost,
    options.privateKey
  );

  const sidechain = new _TronWeb(
    options.fullNode || options.sideFullHost,
    options.solidityNode || options.sideFullHost,
...
const logErrorAndExit = (logger, err) => {
  ...
  if(msg) {
    ...
    if (msg === 'Invalid URL provided to HttpProvider') {
      msg = 'Either invalid or wrong URL provided to HttpProvider. Verify the configuration in your "sun-photon.js"'
    }
