...
var Package = {
  ...
  publish: function(options, callback) {
    ...
    if (!ropsten) {
      return callback(new TruffleError("You need to have a `ropsten` network configured in order to publish to the sun-photon Package Registry. See the following link for an example configuration:" + OS.EOL + OS.EOL + "    https://github.com/tronprotocol/sun-photon" + OS.EOL));
    }
    ...
    Networks.asURIs(options, deployed_networks, function(err, result) {
      ...
      if(!files.length){
        var msg = "Could not locate any publishable artifacts in " +
                  ...
                  "Run `sun-photon compile` before publishing.";
